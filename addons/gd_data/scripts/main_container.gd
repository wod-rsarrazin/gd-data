...
func on_create_column_confirmed(key: String, type: String, editable: bool, settings: Dictionary):
	var sheet_item = get_selected_sheet_item()	
	var sheet = sheet_item.get_metadata(0)
	
	var result: UpdateResult = data.create_column(sheet, key, type, editable, settings)
	if result.is_ko():
		push_error(result.message)
		return
	elif result.is_none():
...
func on_update_column_confirmed(key: String, type: String, editable: bool, settings: Dictionary):
	var sheet_item = get_selected_sheet_item()
	var column_item = get_selected_column_item()
	...
	var result: UpdateResult = data.update_column(sheet, column, key, type, editable, settings)
	if result.is_ko():
		push_error(result.message)
		return
	elif result.is_none():
